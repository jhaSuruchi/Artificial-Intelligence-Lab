...
def get_available_assignments(assignments, completed, shared, deps, gpt_limit, gem_limit):
    """Get list of assignments that can be done given current state."""
    available = []
    for a in assignments:
        ...
        if not deps_done(a, shared, deps):
            continue
        # Each individual assignment must fit in daily limit
        if is_gpt(a):
...
def is_valid_combo_caseA(combo, assignments, gpt_limit, gem_limit):
    """
    Check if combination of assignments respects daily prompt limits.
    In Case A, multiple students work in parallel, but they share the same daily budget.
    """
...
def heuristic(completed, assignments, deps, gpt_limit, gem_limit, num_students, case_type):
    ...
    # Dependency bound (applies to both cases)
    memo = {}
    def depth(a):
        if a in memo:
            return memo[a]
    ...
    if case_type == "A":
        # Case A: N students, each does one assignment per day
        # Also limited by total daily prompts
        simple_bound = ceil(len(remaining) / num_students)
        
        # Prompt bound for Case A
...
def solve_caseA(algo, assignments, deps, gpt_limit, gem_limit, num_students, deadline=None):
    """
    Case A: Each of N students does one assignment per day.
    Up to N assignments can be done per day in parallel.
    The combination must respect total daily prompt limits.
    """
    nodes = 0
    best = float('inf')
    best_path = None
    ...
    if algo == "DFS":
        def dfs(completed, day, shared, path):
            ...
            if not available:
                # No assignments can be done today
                # If we still have incomplete assignments, this is infeasible
                if len(completed) < len(assignments):
                    return  # Dead end - can't complete
                return
...
# =========================================================
# CASE B: Multiple assignments per day, prompt-limited
# =========================================================

def solve_caseB(algo, assignments, deps, gpt_limit, gem_limit, num_students, deadline=None):
    """
    Case B: Students can do multiple assignments per day.
    Limited by group's total daily prompt budget.
    """
    nodes = 0
    best = float('inf')
    best_path = None
    ...
    for algo in ["DFS", "DFBB", "ASTAR"]:
        ...
        if case_type == "A":
            days, seq, nodes = solve_caseA(algo, assignments, deps, 
                                          gpt_limit, gem_limit, num_students)
        else:
            ...
            if seq:
                ...
                for day_num, day_assignments in seq:
                    if isinstance(day_assignments, list):
                        # Case A: show all parallel assignments
...
def query2(case_type, assignments, deps, deadline, c1, c2, num_students):
    ...
    print(f"Total prompts: ChatGPT={total_gpt}, Gemini={total_gem}")
...
def main():
    if len(sys.argv) < 5:
        print("Usage:")
        print("  Query 1: python script.py <file> <case> <N> 1 <gpt> <gem>")
        print("  Query 2: python script.py <file> <case> <N> 2 <deadline> <c1> <c2>")
        print("\nExample:")
        print("  python script.py input.txt A 3 1 5 5")
        print("  python script.py input.txt B 2 2 8 2 3")
        sys.exit(1)
